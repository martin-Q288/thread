...
<div class="card"><h3>게시물</h3><p class="muted">draft 상태의 게시글과 첫 댓글은 직접 수정한 뒤 저장하고 게시할 수 있습니다.</p><div id="posts"></div></div>
</div>
<script>
...
async function refresh(){
  ...
  posts.innerHTML=(po||[]).map(p=>{
    ...
    if(p.status==='draft'){
      return `<div class="card"><span class="badge">${esc(p.hook_type||'hook')}</span> <b>${esc(p.hook||'')}</b><label class="muted">게시글 본문</label><textarea id="postBody${p.id}" class="postEditor">${esc(p.body||'')}</textarea><label class="muted">첫 댓글</label><textarea id="postComment${p.id}" class="commentEditor">${esc(p.first_comment||'')}</textarea>${research}<div class="muted">상태: draft</div><div class="editActions"><button class="saveBtn" onclick="savePost(${p.id})">수정 내용 저장</button><button onclick="publishPost(${p.id})">Threads에 게시 + 첫 댓글</button></div></div>`;
    }
    return `<div class="card"><span class="badge">${esc(p.hook_type||'hook')}</span> <b>${esc(p.hook||'')}</b><pre>${esc(p.body||'')}</pre><pre>${esc(p.first_comment||'')}</pre>${research}<div class="muted">상태: ${esc(p.status||'')}</div></div>`;
  }).join('')||'게시물 없음';
}
...
async function savePost(id){const body=document.getElementById('postBody'+id)?.value||'';const first_comment=document.getElementById('postComment'+id)?.value||'';const r=await api('/api/posts.php',{method:'POST',body:JSON.stringify({id,body,first_comment})});if(r.error)return alert(r.message||r.error);alert('수정 내용 저장 완료');refresh()}
async function publishPost(id){if(!confirm('현재 저장된 본문과 첫 댓글을 Threads에 게시할까요?'))return;const r=await api('/api/publish.php',{method:'POST',body:JSON.stringify({post_id:id})});if(r.error)alert(r.message||r.error);else{alert(r.warning?'게시 완료 · '+r.warning:'게시 완료');refresh()}}
